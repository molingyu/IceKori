...
namespace Assets.Plugins.IceKori.Syntax.Std.Math
{
    public class Min : BaseExpression
    {
        ...
        public Min(BaseExpression a, BaseExpression b)
        {
            A = a;
            B = b;
        }
        ...
        public override BaseExpression Reduce(Enviroment env)
        {
            ...
            {
                return new IceKoriFloat(Mathf.Min(((IceKoriFloat)A).Value, ((IceKoriInt)B).Value));
            }
            if (B.ID == IceKoriBaseType.Float)
            {
                return new IceKoriFloat(Mathf.Min(((IceKoriFloat)A).Value, ((IceKoriInt)B).Value));
            }
            return new IceKoriFloat(Mathf.Min(((IceKoriInt)A).Value, ((IceKoriInt)B).Value));
